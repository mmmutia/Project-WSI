     ...
     <br></br>
      ...
      <div class="container">
        <table class="table-bordered" style="width: 100%;">
          ...
          <?php
          $iduser = $data['id_user'];
          $query_mysql2 = mysqli_query($koneksi,"SELECT pemesanan_rumah.nama_pemesan, proggres.tanggal, proggres.id, proggres.id_pemesanan, proggres.status, proggres.keterangan, proggres.foto FROM proggres INNER JOIN pemesanan_rumah ON pemesanan_rumah.id_pemesanan_rumah=proggres.id_pemesanan where proggres.id_user = '$iduser'");
          $no = 1;
          ...
          <div class="modal-body-add">
            ...
            <div class="form-group">
                ...
                <option value='#'> Pilih Id user</option>
                  ...
                  while ($row = mysqli_fetch_array($query)) {
                    echo "<option value=$row[id_user]> $row[user_fullname]</option>";
                  }
                  ?>
...
if (isset($_POST['add-proggres'])) {
  ...
  $query    = "INSERT INTO `proggres` (`id_pemesanan`,`status`, `keterangan`, `foto`) VALUES ('$idpemesanan', '$statuspemesanan', '$keterangan', '$fotoadd')";
  ...
  if ($result) {
    echo "<script>
		Swal.fire({title: 'Data Berhasil Disimpan',text: '',icon: 'success',confirmButtonText: 'OK'
			...
			{window.location = 'proggres_user.php';}
		})</script>";
  } else {
...
if (isset($_POST['edit-user'])) {
  if (isset($_POST['edit-user'])) {
    if ($fotoedit == "") {
      $sql = mysqli_query($koneksi, "UPDATE `proggres` SET id_pemesanan='$idpemesananedit',status='$statuspemesananedit',keterangan='$keteranganedit' WHERE id='$id'");
      ...
      $sql = mysqli_query($koneksi, "UPDATE `proggres` SET id_pemesanan='$idpemesananedit',status='$statuspemesananedit',keterangan='$keteranganedit', foto = '$fotoedit' WHERE id='$id'");
      // header('location:users.view.php');
      echo "<script>
            Swal.fire({title: 'Data Berhasil Diubah',text: '',icon: 'success',confirmButtonText: 'OK'
            }).then((result) => {if (result.value)
                {window.location = 'proggres_user';}
            })</script>";
    }
  } else {
    echo "<script>
